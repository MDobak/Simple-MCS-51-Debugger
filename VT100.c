...
void vtprintf(const char *format, ...)
{
  char cbuf[1024];
  va_list va;
...
static int ProcessBracket(int Start)
{
  ...
  case 'm':
    ...
    if (numargs < 1) {
      /* FIX - MDobak
       * Chenged from args[i]-30 to args[i]-40.
       */
...
static int ProcessBuffer(void)
{
  ...
  while (Start < BufLen) {
    ...
    /* FIX - MDobak
     * Cast cBuffer to unsigned char.
     */
    /* Since we null-terminated, null < 27 and we have a termination condition */
    while (((unsigned char)cBuffer[End] > 27)||(cBuffer[End] == 10)||(cBuffer[End] == 13))
      End++;
 ...
 */
int
vtProcessedTextOut(const char *cbuf, int count)
{
